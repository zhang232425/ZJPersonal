...
// @IBDesignable关键字用来声明一个类是可以被设计的，可以实时渲染在interface builder 上
@IBDesignable
open class GIFImageView: UIImageView {
    ...
    open override var isHighlighted: Bool {
        didSet {
            onSetHighlighted?(isHighlighted)
        }
    }
    ...
    open override func layoutSubviews() {
        ...
        if let data = animatedImageData,
           let animation = UIImage.animatedImage(withData: data) {
